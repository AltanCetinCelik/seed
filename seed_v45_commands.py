def handle_v45_command(command):
    ...
    if cmd in {"/control-plane", "/control-plane-open", "/open-control-plane"}:
        print("\n=== SEED CONTROL PLANE ===")
        print("URL: http://127.0.0.1:8790")
        print("If it is not running, use /service-start and enter: control-plane")
        try:
            import webbrowser
            webbrowser.open("http://127.0.0.1:8790")
            print("Opened Control Plane in browser.")
        except Exception as error:
            print(f"Could not auto-open browser: {error}")
        return "handled"
