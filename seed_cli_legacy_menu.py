...
def talk_to_seed():
    ...
    print("\n=== TALK TO SEED ===")
    print("You can talk naturally. Try: check yourself, open dashboard, show models, what should we improve next.")
    print("Say 'exit' or use /exit to return to the main menu.")
    ...
    while True:
        ...
        try:
            ...
            print("\n\nSeed chat interrupted. Returning to main menu.")
            return
        except EOFError:
        ...
        # v30.1: support pasted multi-line slash commands.
        if "\n" in user_message and all(
            line.strip().startswith("/") for line in user_message.splitlines() if line.strip()
        ):
            for command_line in [line.strip() for line in user_message.splitlines() if line.strip()]:
